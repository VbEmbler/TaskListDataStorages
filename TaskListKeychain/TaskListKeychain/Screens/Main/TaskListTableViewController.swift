...
class TaskListTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        taskList = keychainManager.getTasks()
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            keychainManager.removeTask(index: indexPath.row)
            taskList = keychainManager.getTasks()
    ...
    private func showAddEditView(isEdit: Bool, indexPath: IndexPath = [], taskName: String) {
        ...
        if isEdit {
            ...
            editAddFunctionName = keychainManager.isTaskEdited
            indexPathRow = indexPath.row
        } else {
            editAddFunctionName = keychainManager.isAddedTask
        ...
        let taskAction = UIAlertAction(title: addEditButtonTitle, style: .cancel, handler: { _ in
            ...
            if editAddFunctionName(indexPathRow, taskName) {
                self.taskList = self.keychainManager.getTasks()
